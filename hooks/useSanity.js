  ...
  useFetch(async () => {
    try {
      ...
      res = res.map((item) => {
        ...
        if (morphed._createdAt)
          morphed._createdAt = formatDate(morphed._createdAt)
        if (morphed._updatedAt)
          morphed._updatedAt = formatDate(morphed._updatedAt)

        // alternate specific keys of specific types
        if (morphed.publishedAt)
          morphed.publishedAt = formatDate(morphed.publishedAt)
        return morphed
      })
      ...
      if (data.value) {
        ...
        if (rslv === null) {
          ...
          if (dv.metadata && dv.metadata.metaImage) {
            ...
            value = sanityImgUrl(
              dv.coverImage || dv.cartoonImage || dv.logoImage || dv.mainImage
            )
          }
        } else {
          value = rslv
